# Claude Code Project Instructions — aws-sigv4-kotlin

## Project Overview

This is **aws-sigv4-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`aws-sigv4`](https://crates.io/crates/aws-sigv4).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/aws-sigv4/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/aws-sigv4>
